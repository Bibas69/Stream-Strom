...
const WatchPage = () => {
    ...
    const getTrailers = async () => {
      try {
        const res = await axios.get(
          `${getBackendUrl()}/api/v1/${contentType}/${id}/trailers`,
  ...
  useEffect(() => {
    const getSimilarContent = async () => {
      try {
        const res = await axios.get(
          `${getBackendUrl()}/api/v1/${contentType}/${id}/similar`,
          { withCredentials: true }
        );
  ...
  useEffect(() => {
    const getContentDetails = async () => {
      try {
        const res = await axios.get(
          `${getBackendUrl()}/api/v1/${contentType}/${id}/details`,
          { withCredentials: true }
        );
  ...
  if (loading) return <div className='min-h-screen bg-black p-10'><WatchPageSkeleton /></div>;
