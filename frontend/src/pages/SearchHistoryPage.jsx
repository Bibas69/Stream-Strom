...
import getBackendUrl from "../utils/getBackendUrl"; // <-- import helper
...
const SearchHistoryPage = () => {
  ...
  useEffect(() => {
    const getSearchHistory = async () => {
      try {
        const res = await axios.get(`${getBackendUrl()}/api/v1/search/history`, {
          withCredentials: true, // include cookies if backend uses auth
        });
  ...
  const handleDelete = async (entry) => {
    try {
      await axios.delete(`${getBackendUrl()}/api/v1/search/history/${entry.id}`, {
